...
// `data-elkey` tags each element's root so SlideCanvas can start a drag on it
// (delegated pointerdown) and address its position offset by the stable key.
const DRAG_CURSOR: CSSProperties = { cursor: "grab", touchAction: "none" };
...
export function ElementView({ e, slideId }: { e: Element; slideId: string }) {
  ...
  if (e.kind === "image") {
    return <img src={e.path} data-elkey={e.key} style={{ ...box(e), objectFit: "contain", ...DRAG_CURSOR }} alt="" />;
  }
